...
def draw_segment(year, circuit, session, start_time, end_time, driver_number, factor, colour, visualize, turtle):
    driver_x, driver_y, utctimestamps, centraltimestamps = r.get_position_data(year, circuit, session, driver_number, start_time, end_time)
...
def draw_lap(session, circuit, year, lap_number, driver, factor, colour, turtle):
    driver_x, driver_y = r.get_lap_positions(year, circuit, session, driver, lap_number)
    processed_data = n.normalize([driver_x, driver_y], factor)
    driver_x = processed_data[0]
    driver_y = processed_data[1]
    turtle.color(colour)
